    ...
    <div className={`${baseClasses} ${hoverClasses} ${variantClasses[variant]} ${className}`}>
      ...
      <div className="relative overflow-hidden">
        {imageUrl && !imageError ? (
          <div className="relative">
            {imageLoading && (
              <div className="absolute inset-0 bg-gradient-to-br from-gray-100 to-gray-200 animate-pulse flex items-center justify-center z-10">
                <div className="w-10 h-10 border-3 border-wingzimpex-brand border-t-transparent rounded-full animate-spin"></div>
              </div>
            )}
            <img
              src={imageUrl}
            ...
            {showHoverEffects && variant === 'homepage' && (
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                <Link
                  href={slug}
                  className="bg-white text-wingzimpex-brand px-6 py-3 rounded-xl font-semibold transition-all duration-300 transform translate-y-4 group-hover:translate-y-0 shadow-xl hover:shadow-2xl hover:bg-wingzimpex-brand hover:text-white"
                >
                  View Details
                </Link>
              </div>
          ...
          <div className="h-56 bg-gradient-to-br from-wingzimpex-brand/5 via-wingzimpex-brand/10 to-wingzimpex-brand/15 flex items-center justify-center">
            <div className="w-20 h-20 bg-gradient-to-br from-wingzimpex-brand/20 to-wingzimpex-brand/30 rounded-full flex items-center justify-center shadow-lg">
        ...
        {showCategoryBadge && (
          <div className="absolute top-4 left-4">
            <span className="bg-wingzimpex-brand text-white px-4 py-2 rounded-full text-xs font-semibold shadow-lg">
              Service
            </span>
          </div>
        )}
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="mt-auto">
          <div className="flex justify-between items-center">
            ...
            <Link
              href={slug}
              className="inline-flex items-center bg-wingzimpex-brand text-white px-4 py-2 rounded-lg font-semibold hover:bg-wingzimpex-brand-light transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
            >
              Learn More
